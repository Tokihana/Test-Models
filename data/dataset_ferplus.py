...
class FERDataSet(data.Dataset):
    def __init__(self, path, dataidxs=None, train=True, transform=None, basic_aug=False, download=False):
        self.train = train
        ...
        NAME_COLUMN = 0
        LABEL_COLUMN = 1
        df = pd.read_csv(os.path.join(self.path, 'label.txt'), sep=' ', header=None)
        if self.train:
        ...
        for f in file_names:    
            #f = f.split(".")[0]
            #f = f + "_aligned.jpg"
            path = os.path.join(self.path, 'image', f)
